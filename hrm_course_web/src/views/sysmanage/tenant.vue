<template>
	<section>
		...
		<el-table :data="tenants" v-loading="listLoading" highlight-current-row  style="width: 100%;">
			...
			<el-table-column prop="companyNum" label="标识">
			</el-table-column>
			<el-table-column prop="address" label="地址" >
			</el-table-column>
			<el-table-column prop="registerTime" label="入驻时间" >
			</el-table-column>
			<el-table-column prop="state" label="状态">
				<template slot-scope="scope">
					<span v-if="scope.row.state!=null && scope.row.state==false" style="color: red">停用</span>
					<span v-else>正常</span>
				</template>
			</el-table-column>
			<el-table-column prop="manager.username" label="管理员">
				<template slot-scope="scope">
			...
			<el-form :model="tenant" label-width="80px" :rules="formRules" ref="tenant">
				...
				<el-form-item label="状态" prop="state">
					<el-radio-group v-model="tenant.state" >
						<el-radio checked="checked" :label=true>正常</el-radio>
	...
	export default {
		...
		methods: {
            ...
            getTenants(){
				...
				//分页查询
                this.$http.patch("/sysmanage/tenant",para) //$.Post(.....)
                    .then(result=>{
