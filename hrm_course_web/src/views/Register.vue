  ...
  <div>
    ...
    <el-form :model="tenant" ref="tenantForm" :rules="formRules" label-position="left" label-width="100px" class="demo-ruleForm login-container">
      ...
      <el-form-item prop="logo" label="机构Logo">
        <!--&lt;!&ndash;<el-input type="text" v-model="tenant.logo" auto-complete="off" placeholder="请输入logo！"></el-input>&ndash;&gt;-->
        <!--<el-upload-->
                <!--class="upload-demo"-->
      ...
      <el-form-item prop="username" label="机构账号">
        <el-input type="text" v-model="adminUser.username" auto-complete="off" placeholder="请输入账号！"></el-input>
      </el-form-item>
      <el-form-item prop="tel" label="手机号码">
        <el-input type="text" v-model="adminUser.tel" auto-complete="off" placeholder="请输入电话！"></el-input>
      </el-form-item>
      <el-form-item prop="email" label="电子邮件">
        <el-input type="text" v-model="adminUser.email" auto-complete="off" placeholder="请输入邮件！"></el-input>
      </el-form-item>
      <el-form-item prop="password" label="密码">
        <el-input type="password" v-model="adminUser.password" auto-complete="off" placeholder="请输入密码！"></el-input>
      </el-form-item>
      <el-form-item prop="comfirmPassword" label="确认密码">
        <el-input type="password" v-model="adminUser.comfirmPassword" auto-complete="off" placeholder="请输入确认密码！"></el-input>
      </el-form-item>
      <el-form-item prop="meals" label="确认密码">
...
<script>
    export default {
        ...
        methods: {
                ...
                this.$refs.tenantForm.validate((valid) => {
                    ...
                    if (valid) {
                        this.$confirm('确认入驻吗？', '提示', {}).then(() => {
                            ...
                            para.adminUser = this.adminUser;
                            console.log(para);
                            //判断是否有id有就是修改,否则就是添加
                            this.$http.post("/sysmanage/tenant",para).then((res) => {
                                if(res.data.success){
                                    this.$message({
                                        message: '操作成功!',
                                        type: 'success'
            ...
            getMeals(){
                this.$http.get("/sysmanage/meal").then((res) => {
                    this.meals = res.data;
                    console.log(res)
                })
            }
